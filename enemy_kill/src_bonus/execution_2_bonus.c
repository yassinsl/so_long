...
/*   Updated: 2025/02/17 13:38:02 by ylahssin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void draw_enemy(t_game*g, t_texture *t, t_norm norm)
{    
	mlx_put_image_to_window(g->mlx_ptr, g->mlx_win,
			t->enemy_right[g->current_frame_up], norm.b * WIDTH, norm.a * HEIGHT);
}
static void	ft_put_tile(t_game *g, t_texture *t, char tile, t_norm norm)
{
	if (tile == CL_CHAR)
		mlx_put_image_to_window(g->mlx_ptr, g->mlx_win,
			t->goins[g->current_frame_goins], norm.b * WIDTH, norm.a * HEIGHT);
	else if (tile == CL_WALL && ((norm.a == 0 || norm.a == g->rows - 1) || norm.b == 0 || norm.b == g->cols - 1))
		mlx_put_image_to_window(g->mlx_ptr, g->mlx_win,
			t->wall[g->current_frame], norm.b * WIDTH, norm.a * HEIGHT);
	else if (tile == CL_WALL && !((norm.a == 0 || norm.a == g->rows - 1) || norm.b == 0 || norm.b == g->cols - 1))
		mlx_put_image_to_window(g->mlx_ptr, g->mlx_win,
			t->simple_wall, norm.b * WIDTH, norm.a * HEIGHT);
	else if (tile == FLOOR_CHAR)
		mlx_put_image_to_window(g->mlx_ptr, g->mlx_win,
			t->space, norm.b * WIDTH, norm.a * HEIGHT);
	else if (tile == 'T')
		mlx_put_image_to_window(g->mlx_ptr, g->mlx_win,
			t->traps_all[g->current_frame_traps], norm.b * WIDTH, norm.a * HEIGHT);
	else if (tile == 'X')
        draw_enemy(g, t, norm);   
}
...
void	ft_render_map(t_game **game, t_texture *t, int is_finish, int key)
{
	...
	while (++norm.a < g->rows)
	{
		...
		while (++norm.b < g->cols)
		{
			...
			else if (tile == EXIT_CHAR &&!(g->found_collect == g->total_collect))
				mlx_put_image_to_window(g->mlx_ptr, g->mlx_win,
